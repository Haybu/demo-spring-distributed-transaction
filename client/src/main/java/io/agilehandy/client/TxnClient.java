...
public class TxnClient implements ApplicationRunner {
	...
	public void run(ApplicationArguments args) throws Exception {
		...
		request1.setJobId(UUID.fromString(String.valueOf(jobId)));
		request1.setFileId(UUID.fromString("10"));
		...
		request2.setJobId(UUID.fromString(String.valueOf(jobId)));
		request2.setField1("f1");
		request2.setField2("f2");
		request2.setRecordId(UUID.fromString("20"));

		BCSubmitRequest request3 = new BCSubmitRequest();
		request3.setJobId(UUID.fromString(String.valueOf(jobId)));
		request3.setContent("some bc content".getBytes());
		request3.setContentId(UUID.fromString("30"));
